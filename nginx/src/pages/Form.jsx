...
import { notifySuccess as sucesso } from '../utils/Toast';
import { notifyError as alerta } from '../utils/Toast';
import { showConfirm } from '../utils/alerta';

const Form = () => {
  // Registro agora passa sempre por /auth/register -> /usuario/register via Nginx
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Simular API call
      const registroSucesso = await AuthService.register(formData);

      if (registroSucesso) {
        sucesso('Usuário cadastrado com sucesso!');
        showConfirm('Deseja fazer login agora?', () => {
      ...
      alerta(error?.message ? `Erro ao cadastrar usuário: ${error.message}` : 'Erro ao cadastrar usuário');
    } finally {
      setLoading(false);
    }
  };
